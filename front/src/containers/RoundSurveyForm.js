...
const replaceNicksInSurvey = (
  ...
  users
    ...
    .forEach((user, index) => {
      ...
      if (newMessage !== message) {
        names.append(user._id.toString());
        message = newMessage;
      }
    });
  newMessage = message.replace(
  ...
  if (newMessage !== message) {
    names.append(currentUser._id.toString());
    message = newMessage;
  }
  messageContainer = [message, names];
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    const component =
      orderedQuestions[i].type === "select"
        ...
        ? renderField
        : renderTextArea;
    const messageContainer = replaceNicksInSurvey(
    ...
    items.push(
      <div key={i} className="form__form-group">
        <label className="form__form-group-label">{messageContainer[0]}</label>
