...
class PostSurveyForm extends React.Component {
  ...
  componentDidMount() {
    ...
    batch.rounds.forEach((round, index) => {
      ...
      const team = round.teams.find(x =>
        x.users.some(y => y.user.toString() === userId)
      );
      team.users.forEach(user => {
        ...
        if (
          (user.user.toString() !== userId &&
            !(
              !batchUser.isActive && batchUser.kickedAfterRound <= index + 1
            )) ||
          (singleTeam &&
            index === surveyRounds[1] &&
            expRound1ActiveUsers.indexOf(batchUser) > -1)
        ) {
          // for correct rosters of surveyRounds
          uOptions.push({
          ...
          roundRoster =
            ...
            (batch.maskType === "unmasked"
              ? batch.users.find(x => x.user === user.user).nickname
              : user.nickname) +
            " ";
        }
      });
      if (singleTeam) {
        for (let i = 0; i <= index; ++i) {
          // in order to make the order of different rounds different from each other
    ...
    const actualPartnerName = singleTeam
      ? singleTeamInfo.actualPartnerName
      : "na";
    let sOptions;
    try {
      sOptions = singleTeam
        ? this.nicksFromRoster(
            this.state.roster,
            singleTeamInfo.roundsForSurvey[1]
          ).map((x, ind, arr) => {
            return {
              value: `${x} ${actualPartnerName} ${arr.length}`,
              label: x
            };
          })
        : [{ value: "na", label: "na" }];
    } catch (e) {
      sOptions = [{ value: "na", label: "na" }];
    }
    this.setState({
      qOptions: qOptions,
      uOptions: uOptions,
    ...
    if (singleTeam) {
      this.setState({
        firstNick: singleTeamInfo.expPersonRound1Nick,
        roundsForSurvey: singleTeamInfo.roundsForSurvey
      });
    }
  }
  ...
  generateSingleTeamInfo() {
    ...
    const userId = this.props.user._id.toString();
    const [expRounds, worstRounds] = [
      batch.expRounds.map(x => x - 1),
      batch.worstRounds.map(x => x - 1)
    ];
    ...
    if (batch.dynamicTeamSize) {
      ...
      nonExpRounds = allRounds.filter(
        x => expRounds.indexOf(x) < 0 && worstRounds.indexOf(x) < 0
      );
    }

    let roundsForSurvey = [];
    roundsForSurvey.push(nonExpRounds[0], nonExpRounds[1]);
    const expPersonRound1 = batch.rounds[
      roundsForSurvey[0]
    ].teams[0].users.find(
      x =>
        x.user.toString() !== userId &&
        !batch.users.find(y => y.user.toString() === x.user.toString())
          .kickedAfterRound
    ); // finds a person in round N which wasn't kicked at all
    let actualPartnerName;
    try {
      actualPartnerName = batch.rounds[roundsForSurvey[1]].teams[0].users.find(
        x => x.user === expPersonRound1.user
      ).nickname;
    } catch (e) {
      console.log(e);
      return {};
    ...
    return {
      expPersonRound1Nick: expPersonRound1.nickname,
      roundsForSurvey: roundsForSurvey,
      actualPartnerName: actualPartnerName
    };
  }
  ...
  render() {
    ...
    batch.tasks.map(task => {
      surveysTotal += task.hasMidSurvey + task.hasPreSurvey;
    });
                ...
                <div className="form__panel">
                  <div className="form__form-group">
                    {batch.withRoster && (
                      <div>
                        ...
                        {this.state.roster.map((round, index) => {
                          return (
                            <p>
                              Round {index + 1}: {this.state.roster[index]}
                            </p>
                          );
                        })}
                      </div>
                    ...
                    <label className="form__form-group-label">
                      <p>
                        Surveys completed: {batch.surveyCounter}/{surveysTotal}
                      </p>
                      <br />
                    </label>
                    {batch.teamFormat !== "single" && (
                      <div>
                        <label className="form__form-group-label">
                          <p>
                            You actually worked with the same team in some of
                            the previous rounds, though their names may have
                            appeared different.
                          </p>
                          <p>
                            In which rounds do you think that you worked with
                            the same people?
                          </p>
                        </label>
                  ...
                  {batch.teamFormat !== "single" && (
                    <div>
                      ...
                      >
                        <label className="form__form-group-label">
                          What partners do you prefer to work with in the
                          future?
                        </label>
                        <div className="form__form-group-field">
                          <Field
                  ...
                  {batch.teamFormat === "single" && (
                    <div className="form__form-group">
                      <label className="form__form-group-label">
                        We didn't tell you this, but in rounds{" "}
                        {roundsForSurvey[0] + 1} and {roundsForSurvey[1] + 1}{" "}
                        you worked with the same people. Their names appeared
                        differently between these two rounds. In team{" "}
                        {roundsForSurvey[0] + 1}, one partner's name was{" "}
                        {this.state.firstNick}. What was their name in round{" "}
                        {roundsForSurvey[1] + 1}?
                      </label>
                      <div className="form__form-group">
                        <Row>
                      ...
                      <div className="form__form-group">
                        ...
                        <div className="form__form-group-field">
                          <Field
                            name="singleTeamQuestion.result1"
                            component={renderField}
                            type="text"
                          />
                        </div>
