    ...
    template.tasks.forEach(task => {
        if (task.hasPreSurvey) task.preSurvey.forEach(preSurvey => {
            if (preSurvey.type === 'select') {
                preSurvey.selectOptions = preSurvey.options.map((x, index) => {
                    return {value: index, label: x.option}
                })
            }
            return preSurvey;
        })
        if (task.hasMidSurvey) task.survey.forEach(survey => {
            if (survey.type === 'select') {
                survey.selectOptions = survey.options.map((x, index) => {
                    return {value: index, label: x.option}
                })
            }
            return survey;
        ...
        if (task.polls && task.polls.length) {
            task.polls.forEach(poll => {
                if (poll.options && poll.options.length) {
                    poll.selectOptions = poll.options.map((y, index) => {
    ...
    if (template.hasPostSurvey) {
        template.postSurvey.forEach(postSurvey => {
            if (postSurvey.type === 'select') {
                postSurvey.selectOptions = postSurvey.options.map((x, index) => {
                    return {value: index, label: x.option}
                })
            }
        })
