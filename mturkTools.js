...
const assignQuals = (user, qual) => {
    ...
    mturk.associateQualificationWithWorker(assignQualificationParams, function (err, data) {
        if (err) console.log("Error assigning ", qual.QualificationTypeId, " to ", user); // an error occurred
...
const unassignQuals = (user, qual, reason) => {
    ...
    mturk.disassociateQualificationFromWorker(assignQualificationParams, function (err, data) {
        if (err) console.log("Error unassigning ", qual.QualificationTypeId, " from ", user); // an error occurred
...
const launchBang = (callback) => {
    ...
    let HITTitle = 'Estimate quantities - bonus up to $' + (hourlyWage) + ' / hour (' + time + ')';
    let description = 'Work in groups to estimate values without external resources. This task will take approximately ' + Math.round((roundMinutes * numRounds) + 10) + ' minutes. There will be a compensated waiting period, and if you complete the entire task you will receive a bonus of $' + bonusPrice + '.';
    ...
    let keywords = 'estimate, quantities, guess, group, values';
    ...
    setTimeout(() => {
        if (hitsLeft > 0 && !taskStarted) {
            ...
            let params2 = {
                Title: HITTitle,
                Description: 'Work in groups to estimate values without external resources. This task will take approximately ' + Math.round((roundMinutes * numRounds) + 10) + ' minutes. There will be a compensated waiting period, and if you complete the entire task you will receive a bonus of $' + bonusPrice + '.',
                ...
                Keywords: 'estimate, quantities, guess, group, values',
