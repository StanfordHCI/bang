...
const subdomain =
  process.env.ENDPOINT === "mturk-requester" ? "worker" : "workersandbox";
...
const params = {
  ...
  Description:
    "Temporary task for people who previously worked with us and did not receive compensation.",
...
function getUsersBatch(repayUserWorkerID, i) {
  ...
  if (foundUserInDB == false)
    console.log(
      "Could not find worker " + repayUserWorkerID + " in our database"
    );
}
...
function didBatchCrash(repayUserWorkerID, batchID, i) {
        ...
        if (batch.batchID == batchID) {
          ...
          if (batch.batchComplete == false) {
            ...
            console.log(
              "Batch for worker " +
                repayUserWorkerID +
                " was complete. No repay HIT was created."
            );
          }
        }
      });
  ...
  if (foundBatchInDB == false)
    console.log(
      "Could not find batch with batchID " + batchID + " in our database."
    );
}
