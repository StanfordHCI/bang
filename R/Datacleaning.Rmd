...
rm(list=ls())
getwd()
path <- "/Users/allieblaising/desktop/bang/.data/1534356049092"
files <- dir(path, pattern = "users.json")
data <- files %>%
  map_df(~fromJSON(file.path(path, .), flatten = TRUE))
frame <- na.omit(data)
batchN = "1534356049092"

## function to extract surveys 
...
viability<- extractSurvey(frame,survey)
psychSafety <- extractSurvey(frame, survey2)
data <- merge(psychSafety, viability, by=c("id", "round"))
...
g + geom_boxplot(varwidth=T, fill="plum") + 
  labs(subtitle="Sum of viability measures grouped by repeat team question for batch number: 1534356049092", 
       x="Repeat team question (1=YES, 2=NO) ",
