...
conditionalPropTreatment <- groupedProportionFractureTreatment %>% filter(condition=="A" | condition=="Ap")
...
ggplot(controlFracture, aes(prop, prop1)) +
  geom_point() +
  geom_jitter() + coord_fixed() + xlim(0.0,1.0) + ylim(0.0,1.0) + labs(title="Team fracture in the first vs. third in control", subtitle="No fracture=0, Fracture=1",  
...
```{r}
fractureLE$abs <- abs(fractureLE2$prop-fractureLE1$prop)
g <- ggplot(fractureLE, aes(abs)) + scale_fill_brewer(palette = "Spectral")
g + geom_histogram(bins=7, 
                   col="pink", 
                   size=.4) + 
  labs(title="Absolute value of change in team fracture proportions in learning effect condition", 
       fill = "Round") + 
  xlab(label="Absolute value of change in team fracture proportions in learning effect condition") + 
  ylab(label="Count") 

treatmentFracture$abs <- abs(treatmentFracture2$prop-treatmentFracture1$prop)
g <- ggplot(treatmentFracture, aes(abs)) + scale_fill_brewer(palette = "Spectral")
g + geom_histogram(bins=7, 
                   col="pink", 
                   size=.4) + 
  labs(title="Absolute change in team fracture proportions in treatment condition", 
       fill = "Round") + 
  xlab(label="Absolute change in team fracture proportions in treatment condition") + 
  ylab(label="Count") 

controlFracture$abs <- abs(controlFracture2$prop-controlFracture1$prop)
g <- ggplot(controlFracture, aes(abs)) + scale_fill_brewer(palette = "Spectral")
g + geom_histogram(bins=7, 
                   col="pink", 
                   size=.4) + 
  labs(title="Absolute change in team fracture proportions in control condition", 
       fill = "Round") + 
  xlab(label="Absolute change in team fracture proportions in control condition") + 
  ylab(label="Count") 

treatmentFractureAbsSum <- sum(treatmentFracture$abs) 
controlFractureAbsSum <- sum(controlFracture$abs)

df1 <- data.frame(treatmentFractureAbsSum, controlFractureAbsSum) 

ggplot(df1, aes(x=controlFractureAbsSum, y=treatmentFractureAbsSum)) +
    geom_bar(stat="identity") 

g <- ggplot(test,aes(x, y)) + scale_fill_brewer(palette = "Spectral")
g + geom_bar() + 
  labs(title="Absolute change in team fracture proportions in control condition", 
       fill = "Round") + 
  xlab(label="Absolute change in team fracture proportions in control condition") + 
...
dat <- data.frame(conditionChanges = factor(c("Sum of absolute fracture change in treatment","Sum of absolute fracture change in control"), levels=c("Sum of absolute fracture change in treatment","Sum of absolute fracture change in control")),
    fractureProportionChange = c(13.083, 1.166))

p <- ggplot(data=dat, aes(x=conditionChanges, y=fractureProportionChange)) +
    geom_bar(stat="identity")
p <- ggplotly(p)
p
```
