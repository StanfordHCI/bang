...
g <- ggplot(groupedProportionFractureTreatment, aes(factor(fractureGreater50), mean))
g + geom_boxplot(varwidth=T, fill="plum") + 
  labs(subtitle="Team fracture value vs. mean viability score: fracture >0.50", 
       x="",
       y="Numeric sum of viability measures questions (range: 7-70)") 
...
g <- ggplot(groupedProportionFractureTreatment, aes(factor(fractureGreaterEqual50), mean))
g + geom_boxplot(varwidth=T, fill="plum") + 
  labs(subtitle="Team fracture value vs. mean viability score: fracture >=0.50", 
       x="",
       y="Numeric sum of viability measures questions (range: 7-70)") 
...
controlFracture$fractureGreater50 <- as.numeric(controlFracture$fractureGreater50)
controlFracture1$fractureGreater50 <- as.numeric(controlFracture1$fractureGreater50)
controlFracture1$fractureGreaterEqual50 <- as.numeric(controlFracture1$fractureGreaterEqual50)
...
g <- ggplot(controlFracture, aes(absGreaterEqual50)) + scale_fill_brewer(palette = "Spectral")
g + geom_histogram(bins=7, 
                   col="pink", 
                   size=.4) + 
  labs(title="Absolute change in team fracture score in control condition (fracture >=.50)", 
...
```{r}
ggplot(data=groupedProportionFractureTreatment, aes(fractureGreater50)) + 
  geom_bar(breaks=seq(0, 1, by=0.20), 
                 col="red", 
                 fill="green", 
                 alpha=.2) + labs(title="Fracture ratios: treatment condition, fracture >.50") + facet_grid(condition ~ results.condition)

ggplot(data=groupedProportionFractureTreatment, aes(fractureGreaterEqual50)) + 
  geom_bar(breaks=seq(0, 1, by=0.20), 
                 col="red", 
                 fill="green", 
                 alpha=.2) + labs(title="Fracture ratios: treatment condition, fracture >=.50") + facet_grid(condition ~ results.condition)
...
individualProportion$manipulationAnswerKey <- substr(individualProportion$results.manipulationCheck, 13, 29) 
...
## Subset only observations with team strings//omit NAs and results that can't be processed: 
...
treatmentManipulation <- cleanManipulation %>% group_by(id, results.condition, manipulationAnswers, manipulationAnswerKey) %>% filter(results.condition=="treatment") %>% 
  summarise(n=n())
sum(ifelse(treatmentManipulation$manipulationAnswers==treatmentManipulation$manipulationAnswerKey,1,0))
## 10 out of 24
```
