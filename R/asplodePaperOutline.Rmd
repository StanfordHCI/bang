...
## Make artifical baseline groups: 

data$results.condition[(data$results.format=="c(1, 1, 2)" | data$results.format=="c(2, 1, 1)") & (data$round=="1" | data$round=="3")]  <- "baseline"
...
baselineFracture2 <- groupedProportionFractureBaseline %>% filter(condition=="B")
baselineFracture <- cbind(baselineFracture1, baselineFracture2)
...
model1 <- glm(fractureGreater50 ~ mean, family = "binomial", data = train)
summary(model1)
...
## Making predictions: 
## What is the probability of fracture given the following team mean viability scores: 40 and 65: 

predict(model1, data.frame(mean = c(40, 65)), type = "response")
...
ggplot(data=groupedProportionFracture, aes(fractureGreaterEqual50, mean)) + 
   geom_point() +
  stat_smooth(method = "lm", col = "red") + labs(main="Scatterplot of fracture value vs. mean for team viability sums")
```
...
ggplot(data=groupedProportion, aes(groupedProportion$prop)) + 
  geom_histogram(breaks=seq(0, 1, by=0.20), 
                 ...
                 alpha=.2) + labs(title="Frequency of team fracture proportions by condition and pattern sequence", 
                                  x="team fracture proportion", y="Count") + facet_grid(results.condition ~ results.format) 
...
groupedProportionFractureSecond <- groupedProportionFracture %>% group_by(results.condition) %>% 
      filter(condition=="Ap") %>% 
      mutate(overallPercent = sum(fractureGreaterEqual50)/nrow(groupedProportionFracture)) %>% 
      summarise(n=n(), overallPercent=unique(overallPercent)) 
...
## Baseline: 
groupedProportionFractureBaseline <- groupedProportionFracture %>% 
filter(results.condition=="baseline")
treatmentFracture1 <- groupedProportionFractureBaseline %>% filter(condition=="A")
treatmentFracture2 <- groupedProportionFractureBaseline %>% filter(condition=="Ap")
treatmentFracture <- cbind(treatmentFracture1, treatmentFracture2)
...
propFractureChangeTreatment <- prop.test(x = c(), n = c(100, 100))

propFractureChangeControl <- prop.test(x = c(), n = c(100, 100))

propFractureChangeBaseline <- prop.test(x = c(), n = c(100, 100))
...
conditionalProbBaseline - groupedProportionFractureBaseline %>% filter(condition=="A" | condition=="Ap")
tally(~fractureGreaterEqual50 | condition, data = conditionalProbControl, format = "proportion")
...
controlFracture$fractureGreaterEqual50 <- as.numeric(controlFracture$fractureGreaterEqual50)
controlFracture$absGreaterEqual50 <- abs(controlFracture$fractureGreaterEqual50-controlFracture1$fractureGreaterEqual50)

baselineFracture2$fractureGreaterEqual50 <- as.numeric(baselineFracture2$fractureGreaterEqual50) 
baselineFracture1$fractureGreaterEqual50 <- as.numeric(baselineFracture1$fractureGreaterEqual50) 
baselineFracture$absEqualGreater50 <- abs(treatmentFracture2$baselineGreaterEqual50-baselineFracture1$fractureGreaterEqual50)
...
baselineFractureEqualGreaterAbsSum <- sum(baselineFractureEqualGreaterAbsSum) 

treatmentAbsChange=c() 
controlAbsChange=c()
baselineAbsChange=c()

## Using baseline as theoretical distribution 
## Treatment: 
