...
task = c("creative", "intellective", "cognative-conflic")
...
## Define and initialize cut-off pozzint for fracture:
teamStats <- teamStats %>%
...
manipulationCheckAccuracyByTeams <- teamStats %>%
  group_by(condition,task) %>%
  summarise(meanCorrect=sum(groupManipulationCheck>=.25),n=n(),accuracy=meanCorrect/n)
...
fractureFirst %>%
  group_by(teamFracture) %>%
  summarise(
    viabilityMean = mean(teamMeanViability)
  )
...
fractureConsistency$viabilityIsUp <- fractureConsistency$continuous > 0
# rm("fractureFirst","fractureSecond")

mean(fractureConsistency$continuous)
mean(fractureConsistency$continuousAbs)

test = fractureConsistency %>% 
  group_by(task,condition,consistent) %>%
  summarise(
    meanContinousAbs = mean(continuousAbs),
    percentUp = mean(viabilityIsUp),
    n = n(),
    fractureRate = mean(mean(teamFracture.first),mean(teamFracture.second))
  )

mean(fractureConsistency$viabilityIsUp)
