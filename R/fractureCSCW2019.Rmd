...
DATA_PATH = "../.data" # This is set to so that we can access data where Bang usually stores it.
MINIMUM_BATCH_NUM = 1552194538000  # Identify the batch number that represents the start of the batches you'd like to include in your analysis. 
...
users_stored = function(item) { any(c("users.json") == dir(paste(DATA_PATH,item,sep="/"))) }
batch_complete = function(item) { any(read_json(paste(DATA_PATH,item,"batch.json",sep="/"), simplifyVector = TRUE)$batchComplete) }
get_task = function(item) { checkTask(read_json(paste(DATA_PATH,item,"batch.json",sep="/"), simplifyVector = TRUE)) }

# Makes a list of batches and performs initial filters on them. This list is used when we look up batchs later  
batch_list <-  as.data.frame(dir(DATA_PATH, pattern = "^[0-9]+$")) %>% 
  transmute(batch = as.numeric(as.character(`dir(DATA_PATH, pattern = \"^[0-9]+$\")`))) %>% 
  filter(batch > MINIMUM_BATCH_NUM) %>%
  filter(sapply(batch, users_stored)) %>% 
  filter(sapply(batch, batch_complete)) %>% 
  mutate(task = sapply(batch, get_task))

# generates a user table that basically stores all the data we have for each user (drawn from the users db)
users <- Reduce(
  function(x,y) merge(x, y, all=TRUE), 
  lapply(batch_list$batch, 
         function(batch) flatten(read_json(paste(DATA_PATH,batch,"users.json",sep="/"), 
                                           simplifyVector=TRUE), 
                                 recursive = TRUE)
         )
  )
...
#   select(contains("demographicsCheck"),mturkId,batch) %>%
#   mutate(
#     age = 2019 - as.numeric(results.demographicsCheck.3),
#     gender = as.factor(results.demographicsCheck.2)
#   ) %>%
#   filter(age < 200) #No old people
#   
# write.csv(userDemographics, file = "userDemographics.csv")
...
psychSafety <- extractBlacklistSurvey(users, 'psychologicalSafety')
...
usersRounds <- manipulationCheck %>%
  ...
  left_join(psychSafety, by=c("id", "batch", "round")) %>%
...
usersRounds <- usersRounds %>%
  ...
  mutate_at(.vars = vars(contains("psychologicalSafety")), funs(factor(., levels = levels)))
usersRounds$viabilityScale <- usersRounds %>%
  select(contains("viabilityCheck")) %>%
  mutate_all(as.numeric) %>%
  rowSums()
usersRounds$psychSafetyScale <- usersRounds %>%
  select(contains("psychologicalSafety")) %>%
  mutate_all(as.numeric) %>%
  rowSums()
usersRounds <- usersRounds %>%
  select(-contains("viabilityCheck"),-contains("psychologicalSafety"))
rm("levels")


# export_user_rounds <- usersRounds %>% 
#   mutate(fracture = results.qFifteenCheck == "Do not keep this team") %>% 
#   select(batch,room,round,id,fracture,contains("viabilityScale"),contains("psychSafetyScale"))
# write.csv(export_user_rounds,"oldUserRounds.csv")
...
teamStats <- usersRounds %>%
  ...
  summarise(n=n(), 
            ...
            teamMeanpsychSafety=mean(psychSafetyScale),
            teamMedianpsychSafety=median(psychSafetyScale),
            groupManipulationCheck=sum(as.numeric(manipulationCheck==manipulation)/n()),
            teamProportionFracture=sum(fracture/n()),
            teamProportionFractureMeta=sum(fractureMeta/n()),
...
# exportRounds = usersRounds %>% 
#   select(-c(teamOrder,manipulationCheck.x.x,rooms.x.x,rooms.y.y,manipulationCheck.y.y))
# 
# write.csv(exportRounds, file = "userRoundsExtra.csv")
# write.csv(teamStats, file = "teamStatsExtra.csv")
...
# write.csv(fractureConsistency, file = "fractureStats.csv")
```

```{r}
# Quick analysis of meta fracture prediction accuracy
data %>% 
  group_by(teamFracture) %>%
  summarise(
    votedForOutcome = sum(teamFracture==fracture),
    predictedOutcome = sum(metaGuessedCorrectly),
    n = n(),
    voteRatio = votedForOutcome/n,
    predictionRatio = predictedOutcome/n
  )
```
