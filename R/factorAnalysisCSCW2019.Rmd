...
- Consistency is key: 
  ...
  - If the above conditions are met, pick a single round for analysis, follow notes for factor analysis in the section below
...
# Research questions: 
1. Number of underlying patterns (factors/components): How many best fit the data? 
2. Scale development: 1. What are the underlying piece? 2. How do the questions group together? 
...
3. Sample size 300 + generally agreed upon as best (CHECK)
...
# How many factors? 
nofactors = fa.parallel(data, fm = "ml", fa = "fa") 
sum(nofactors$fa.values > 1.0) # Old Kaiser criterion 
sum(nofactors$fa.values > .7) # New Kaiser criterion 

# All three criterion suggest 1 factor 
```
# Simple structure w/ a three factor model
...
usersNumeric.fa1
usersNumeric.fa5 <- factanal(usersNumeric, factors = 1, rotation = "varimax", scores = "regression")
head(usersNumeric.fa5$scores)
```
...
threefactor <- fa(usersNumeric,nfactors = 1,rotate = "oblimin",fm="minres")
...
fourfactor <- fa(data,nfactors = 1,rotate = "oblimin",fm="minres")
...
Our goal is to have Tucker Lew Index of Factoring to be greater than 0.90: 
- Our model TLI is 0.971  
2. Residual statistics: 
Our goal is to have root mean square error of approximation and root mean square of the residual <.10
