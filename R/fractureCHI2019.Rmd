...
groupedProportion <- data %>%
  group_by(room, batch, round, condition, results.condition) %>%
  summarise(n=n(), meanViability=mean(viabilityScale), medianViability=median(viabilityScale), proportionBlacklisted=sum(blacklistIndividual=="Do not keep this team")/n())
