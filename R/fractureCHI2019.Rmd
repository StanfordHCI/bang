...
groupedProportion <- data %>%
  ...
  summarise(n=n(), meanViability=mean(viabilityScale), medianViability=median(viabilityScale), proportionFracture=sum(fracture/n()))

# Filter down to only partners (dyads)
groupedProportion <- groupedProportion %>%
  filter(n==2)
data <- groupedProportion %>%
  select(room, batch, round, proportionFracture, condition) %>%
...
teamFracture <- groupedProportion %>%
  ...
  mutate(teamFracture = case_when (proportionFracture<.50 ~ 0, 
                                proportionFracture>=.50 ~ 1))

#MSB TODO next: rejoin with data to add otherFractured
