...
```{r, echo=TRUE}
groupedProportionFracture$fracture <- as.numeric(groupedProportionFracture$fracture) 
resultsRound <- groupedProportionFracture %>% group_by(round) %>% summarise(n=n(),fracture=sum(fracture), percent=fracture/n, se=sqrt(percent*(1-percent)/n)) 
...
normalizedTreatmentChange <- round(treatmentFractureAbsSum/nrow(treatmentFracture),2) 
normalizedControlChange <- round(controlFractureAbsSum/nrow(controlFracture), 2) 
...
## Visualizing change: 
```{r, echo=TRUE}
normalizedControlChangeSe <- round(sqrt(normalizedControlChange*(1-normalizedControlChange)/nrow(controlFracture)), 2) 
normalizedTreatmentChangeSe <- round(sqrt(normalizedTreatmentChange*(1-normalizedTreatmentChange)/nrow(treatmentFracture)), 2)

## Visualize differences: 
dat <- data.frame(NormalizedAbsoluteFractureChange= factor(c("Masked","Unmasked"), levels=c("Masked","Baseline","Unmasked")),
                  fractureValueSwitch = c(normalizedTreatmentChange, normalizedControlChange), SE=c(normalizedTreatmentChangeSe, normalizedControlChangeSe)) 

p <- ggplot(data=dat, aes(x=NormalizedAbsoluteFractureChange, y=fractureValueSwitch)) +
  geom_bar(stat="identity", fill="#2853C2", alpha=0.7)+
  scale_y_continuous(labels = scales::percent_format(10)) + 
  geom_text(aes(label=fractureValueSwitch), vjust=1.6, color="white", size=8)+
  theme_classic() + labs(x="",
       y="% of change in fracture scores") + theme_classic() + theme(text = element_text(size=25),
        axis.text.x = element_text(size=16 )) 
p
```
...
ggplot(individualProportionFracture, aes(x = factor(blacklistIndividual))) + 
    geom_bar(fill="#2853C2", alpha=0.7, aes(y = (..count..)/sum(..count..))) +
scale_y_continuous(labels = scales::percent_format(10))  + 
  scale_x_discrete(labels=c("0"="Keep this team", "1"="Do not keep this team")) + labs(y="", x="Answers for individual team fracture question") + facet_grid(.~results.condition, labeller = labeller(tbl)) + theme_classic() + theme(text = element_text(size=20),
        axis.text.x = element_text( )) 

ggplot(individualProportionFracture, aes(x = factor(blacklistMeta))) + 
    geom_bar(fill="#2853C2", alpha=0.7, aes(y = (..count..)/sum(..count..))) +
scale_y_continuous(labels = scales::percent_format(10))  + 
  scale_x_discrete(labels=c("0"="Keep this team", "1"="Do not keep this team")) + labs(y="", x="Answers to team fracture question") +  facet_grid(.~results.condition) + theme_classic() + theme(text = element_text(size=20),
        axis.text.x = element_text(size=16 )) 
```
...
ggplot(individualProportionFracture, aes(x = factor(blacklistIndividual))) + 
    geom_bar(fill="#2853C2", alpha=0.7, aes(y = (..count..)/sum(..count..))) +
scale_y_continuous(labels = scales::percent_format(10))  + 
  scale_x_discrete(labels=c("0"="Keep this team", "1"="Do not keep this team")) + labs(y="", x="Answers to individual fracture question") +  facet_grid(.~results.condition) + theme_classic() 
```
