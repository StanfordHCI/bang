...
extractBlacklistSurvey = function(frame,survey) {
  rounds = seq(1,3) #4 for 4 round data
...
users <- users %>%
  select(-contains("psychologicalSafety"), -contains("undefined"), -contains("blacklist"))


#If Importing old data...
users <- users %>%
  mutate(results.viabilityCheck.1.15 = ifelse(is.na(results.viabilityCheck.1.15),results.viabilityCheck.1.15,
                                          ifelse(results.viabilityCheck.1.15 == "No", "Do not keep this team",
                                                 ifelse(results.viabilityCheck.1.15 == "Yes","Keep this team",results.viabilityCheck.1.15)))) %>%
  mutate(results.qFifteenCheck.1 = ifelse(!is.na(results.qFifteenCheck.1) , results.qFifteenCheck.1, results.viabilityCheck.1.15)) %>%
  mutate(results.viabilityCheck.2.15 = ifelse(is.na(results.viabilityCheck.2.15),results.viabilityCheck.2.15,
                                          ifelse(results.viabilityCheck.2.15 == "No", "Do not keep this team",
                                                 ifelse(results.viabilityCheck.2.15 == "Yes","Keep this team",results.viabilityCheck.2.15)))) %>%
  mutate(results.qFifteenCheck.2 = ifelse(!is.na(results.qFifteenCheck.2) , results.qFifteenCheck.2, results.viabilityCheck.2.15)) %>%
  mutate(results.viabilityCheck.3.15 = ifelse(is.na(results.viabilityCheck.3.15),results.viabilityCheck.3.15,
                                          ifelse(results.viabilityCheck.3.15 == "No", "Do not keep this team",
                                                 ifelse(results.viabilityCheck.3.15 == "Yes","Keep this team",results.viabilityCheck.3.15)))) %>%
  mutate(results.qFifteenCheck.3 = ifelse(!is.na(results.qFifteenCheck.3) , results.qFifteenCheck.3, results.viabilityCheck.3.15)) %>%
  select(-contains("15"), -contains("manipulation"), -contains("Sixteen"), -contains(".4."), -results.qFifteenCheck.4)

users <- users %>%
  select(-contains("results.viabilityCheck.0"))
users <- na.omit(users)
...
usersRounds <- usersRounds %>% mutate(room = ifelse(is.list(room), NA, room))
usersRounds <- na.omit(usersRounds)

users$results.manipulation = TRUE
users$results.manipulationCheck.1 = TRUE
...
usersRounds <- usersRounds %>% mutate(
  team = case_when(
    teamOrder=="c(1, 1, 2)" & round==1 ~ "A",
    teamOrder=="c(1, 1, 2)" & round==2 ~ "Ap",
    teamOrder=="c(1, 1, 2)" & round==3 ~ "B",
    teamOrder=="c(1, 2, 1)" & round==1 ~ "A",
    teamOrder=="c(1, 2, 1)" & round==2 ~ "B",
    teamOrder=="c(1, 2, 1)" & round==3 ~ "Ap",
    teamOrder=="c(2, 1, 1)" & round==1 ~ "B",
    teamOrder=="c(2, 1, 1)" & round==2 ~ "A",
    teamOrder=="c(2, 1, 1)" & round==3 ~ "Ap"
  ))
...
# manipulationCheckAccuracy <- manipulationCheckAccuracy %>% filter(batch > 1537514940000)
...
# last 2 days 1537366991708
# start of final day  1537514940000
# fractureConsistency <- fractureConsistency %>% filter(batch > 1537514940000)
...
consistencyLogit <- glm(data=fractureConsistency, consistent ~ condition, family="binomial")
