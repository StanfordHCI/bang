...
groupedProportion <- data %>%
  ...
  summarise(n=n(), teamMeanViability=mean(viabilityScale), teamMedianViability=median(viabilityScale), teamProportionFracture=sum(fracture/n()))
...
data <- data %>%
  select(-teamProportionFracture, -team, -condition) %>%
  left_join(teamFracture, by=c("room", "batch", "round"))
