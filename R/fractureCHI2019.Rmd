...
# R metaGraph
```{r}
individualProportionMeta <- unique(setDT(individualProportionFracture), by = c('id')) 
individualProportionMeta$correctAnswers <- sum(ifelse(individualProportionMeta$blacklistMeta==individualProportionMeta$fracture,1,0))

metaAnswersCorrect <- round(unique(individualProportionMeta$correctAnswers /nrow(individualProportionMeta)),2) 
metaAnswersSE <- round(unique(sqrt(metaAnswersCorrect*(1-metaAnswersCorrect)/nrow(individualProportionMeta))), 2)  
...
summary(glm(data = didpartnerfracture, partnerFractured.x ~ results.condition.x, family="binomial"))
