...
# onebatch <- flatten(read_json(paste(dataPath,1537471221018,'users.json', sep="/"), simplifyVector=TRUE), recursive=TRUE)
...
#foo <- rbind.fill(userFiles)
#foo2 <- userFiles %>% rbindlist(., fill=TRUE, use.names=TRUE)
#foo3 <- Reduce(function(x,y) merge(x, y, all=TRUE), head(userFiles, 9))
...
# it works here
#testing <- finalRounds %>% select(results.manipulationCheck.1)
#foo <- finalRounds %>% mutate(missingManipulationCheck = is.na(results.manipulationCheck.1), round=unlist(round), room=unlist(room), batch=unlist(batch)) %>% group_by(round, room, batch, missingManipulationCheck) %>% summarise(count = n()) %>% group_by(count, missingManipulationCheck) %>% summarise(metacount=n())

## Subset incomplete cases from viability survey dataframe, use manipulation check b/c required for complete observation:
data <- data[!is.na(data$manipulationCheck.1),]
...
data2 <- finalRounds[!is.na(finalRounds$results.manipulationCheck.1), ]

# AS OF HERE THERE ARE LOTS OF 1s


## Select only variables of interest from final rounds: 
data2 = data2 %>% select(id, batch, room, round, results.condition, results.format, results.manipulation, results.manipulationCheck.1)
...
data2 <- data2 %>% mutate(id=as.character(id), round=as.character(round), batch=as.character(batch)) 
...
data <- data %>% mutate(id=as.character(id), round=as.character(round), batch=as.character(batch))  
...
stats <- stats %>% select(contains("results.viabilityCheck"), id, round, batch, batch, room, blacklistIndividual, blacklistMeta, results.manipulation, results.condition, results.manipulationCheck.1, results.format, condition, sum, median, mean)
