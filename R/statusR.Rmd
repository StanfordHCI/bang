...
rm(list=ls())
setwd("/Users/phoebeyao/Documents/HCI\ Research/bang/R") 
getwd()
dataPath = "../.data"
...
completeStatusBatches = Filter(function(batch) {
  if (any(dir(paste(dataPath,batch,sep="/")) == "batch.json") && (any(dir(paste(dataPath,batch,sep="/")) == "users.json")) ) {
    ...
    return(any(batchData$midSurveyStatusOn == TRUE && batchData$batchComplete == TRUE))
  } 
  return(FALSE)
}, batches)

## Debug: if you get error with flatten, you need to unload tidyverse in package column to the right:  
userFiles = lapply(completeStatusBatches, function(batch) {
...
allConditions <- data[data$batch %in% completeStatusBatches, ]
