...
MINIMUM_BATCH_NUMBER = 1555782062133  # Identify the batch number that represents the start of the batches you'd like to include in your analysis. 
...
usersRounds <- usersRounds %>% select(mturkId, id, batch, round, room)
...
manipulationCheck <- usersRounds %>%
  ...
  select(mturkId, id, batch, room, round, manipulation, manipulationCheck, teamOrder, condition) %>%
...
print(statusSurvey)
print(conflictSurvey)
print(creativeSurvey)
print(satisfactionSurvey)
```
...
usersRounds$satisfactionScale.1 <- usersRounds %>%
  select(contains("satisfactionCheck.1")) %>%
...
usersRounds <- usersRounds %>%
  mutate_at(.vars = vars(contains("satisfactionCheck")), funs(factor(., levels = satisfactionFaceLevels)))
usersRounds$satisfactionScale.2 <- usersRounds %>%
  select(contains("satisfactionCheck.2")) %>%
  mutate_all(as.numeric) %>%
  rowSums()
usersRounds <- usersRounds %>%
  select(-contains("satisfactionCheck.2"))
...
usersRounds$statusScaleSum <- usersRounds %>%
...
teamStats <- usersRounds %>%
  ...
  summarise(n=n(), 
            teamMeanStatusSum=mean(statusScaleSum), 
            teamMedianStatusSum=median(statusScaleSum),
            teamMeanStatus.1=mean(statusScale.1), 
            teamMedianStatus.1=median(statusScale.1),
            teamMeanStatus.2=mean(statusScale.2), 
            teamMedianStatus.2=median(statusScale.2),
            teamMeanStatus.3=mean(statusScale.3), 
            teamMedianStatus.3=median(statusScale.3),
            teamMeanStatus.4=mean(statusScale.4), 
            teamMedianStatus.4=median(statusScale.4),
            teamMeanSatisfaction.1=mean(satisfactionScale.1), 
            teamMedianSatisfaction.1=median(satisfactionScale.1),
            teamMeanSatisfaction.2=mean(satisfactionScale.2), 
            teamMedianSatisfaction.2=median(satisfactionScale.2),
...
usersRounds <- teamStats %>%
  select(room, batch, round, teamProportionFracture, condition, team) %>%
  left_join(usersRounds, by=c("room", "batch", "round", "condition", "team"))

statusData <- teamStats %>%
  select(teamMedianStatus.1, teamMedianStatus.2, teamMedianStatus.3, teamMedianStatus.4)

userRounds$maxStatus = c(1, 2, 3, 4)[apply(statusData, 1, which.max)]
...
```
```{r}
medianStatusUser <- teamStats %>%
  filter(team=="Ap"|team=="A") %>%
  group_by(condition, team, batch, round)

print(medianStatusUser)

medianStatusChanges <- teamStats %>%
  filter(team=="Ap"|team=="A") %>%
  group_by(condition, team) %>%
  summarise(medianStatus = median(teamMedianStatusSum))
...
medianSatisfactionChanges <- teamStats %>%
  filter(team=="Ap"|team=="A") %>%
  group_by(condition, team) %>%
  summarise(medianSatisfaction1 = median(teamMedianSatisfaction.1), medianSatisfaction2 = median(teamMedianSatisfaction.2))

stat <- ggplot(data=medianStatusChanges, aes(x=team, y=medianStatus))+
  geom_bar(stat="identity")+
  facet_wrap(~condition)
con <- ggplot(data=medianConflictChanges, aes(x=team, y=medianConflict))+
  geom_bar(stat="identity")+
  facet_wrap(~condition)
cre <- ggplot(data=medianCreativeChanges, aes(x=team, y=medianCreative))+
  geom_bar(stat="identity")+
  facet_wrap(~condition)
sat1 <- ggplot(data=medianSatisfactionChanges, aes(x=team, y=medianSatisfaction1))+
  geom_bar(stat="identity")+
  facet_wrap(~condition)
sat2 <- ggplot(data=medianSatisfactionChanges, aes(x=team, y=medianSatisfaction2))+
  geom_bar(stat="identity")+
  facet_wrap(~condition)
