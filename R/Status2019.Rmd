...
teamStats <- usersRounds %>%
  ...
  summarise(n=n(), 
            ...
            proportionFemale = sum(gender=="Female")/n())
...
usersRounds <- teamStats %>%
  select(room, batch, round, medianStatus.1, medianStatus.2, medianStatus.3, medianStatus.4, teamProportionFracture, proportionFemale, condition, team) %>%
  left_join(usersRounds, by=c("room", "batch", "round", "condition", "team"))

userScales <- usersRounds %>%
  select(room, round, batch, condition, team, mturkId, age, gender, education, income, latin, race, member.1, member.2, member.3, member.4, medianStatus.1, medianStatus.2, medianStatus.3, medianStatus.4, satisfactionScale, creativeScale, conflictScale, proportionFemale) %>%
...
userStatus <- userScales %>%
  ungroup() %>%
  select(mturkId, room, batch, age, gender, race, income, education, team, condition, medianStatus, proportionFemale) %>%
...
pearsonGenderProportion <- userStatus %>%
  group_by(proportionFemale) %>%
  summarize(rho = cor.test(x=A, y=Ap, method = 'pearson')$estimate, p = cor.test(x=A, y=Ap, method = 'pearson')$p.value)

# Table of true false depending on if leader is the same per team
leaderProb <- userStatus %>%
  group_by(room, batch, condition, proportionFemale) %>%
...
leaderProbStatGender <- leaderProb %>%
  group_by(proportionFemale) %>%
  summarise(
    estimate = prop.test(sum(sameLeader == TRUE), length(sameLeader))$estimate[1],
    SE = sqrt((sum(sameLeader == TRUE)/length(sameLeader)) * (1 - (sum(sameLeader == TRUE)/length(sameLeader)))/length(sameLeader)),
...
pearsonConStatus <- userScales %>%
  group_by(condition) %>%
  summarize(rho = cor.test(x=conflictScale, y=medianStatus, method = 'pearson')$estimate, p = cor.test(x=conflictScale, y=medianStatus, method = 'pearson')$p.value)

# individual age vs individual status: seems like greater status correlates with lower conflict
pearsonAgeStatus <- userScales %>%
  group_by(condition) %>%
  summarize(rho = cor.test(x=age, y=medianStatus, method = 'pearson')$estimate, p = cor.test(x=age, y=medianStatus, method = 'pearson')$p.value)
...
statbox <- ggplot(split_spearmanRankStat, aes(x = condition, y = rho)) +
  geom_boxplot() +
  ggtitle("Status Rank Correlation Between A and Ap")

satbox <- ggplot(split_spearmanRankSat, aes(x = condition, y = rho)) +
  geom_boxplot() +
  ggtitle("Satisfaction and Median Member Status Rank Correlation")

crebox <- ggplot(split_spearmanRankCre, aes(x = condition, y = rho)) +
  geom_boxplot() +
  ggtitle("Creativity and Median Member Status Rank Correlation")

conbox <- ggplot(split_spearmanRankCon, aes(x = condition, y = rho)) +
  geom_boxplot() +
  ggtitle("Conflict and Median Member Status Rank Correlation")
...
pGenderstat <- ggplot(data=leaderProbStatGender, aes(x=proportionFemale, y=estimate))+
  geom_bar(stat="identity")+
  geom_errorbar(aes(ymin=estimate-SE, ymax=estimate+SE), width=.1)+
  ggtitle("Probability of Same Leader Between A and Ap Rounds Per Proportion Females") 
...
hist(userScales$medianStatus, 
     main="Median Status Values Across All Rounds", 
     xlab="Median Status Values", 
     las=1, 
     breaks=20)

hist(userScales$satisfactionScale, 
     main="Satisfaction Values Across All Rounds", 
     xlab="Median Conflict Values", 
     las=1, 
     breaks=20)

hist(userScales$conflictScale, 
     main="Conflict Values Across All Rounds", 
     xlab="Median Conflict Values", 
     las=1, 
     breaks=20)

hist(userScales$creativeScale, 
     main="Creative Values Across All Rounds", 
     xlab="Median Conflict Values", 
     las=1, 
     breaks=20)
...
print(pearsonSatStatus) # correlation between individual satisfaction and status by condition
print(pearsonCreStatus) # correlation between perceived team creativity and status by condition
print(pearsonConStatus) # correlation between percieved team conflict and status by condition
print(pearsonAgeStatus) # no correlation between age and status
