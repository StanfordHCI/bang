...
const createTeams = (teamSize: number, numRounds: number, people: string[]) => {
    ...
    if (teamSize > numRounds+1) throw "Team size is too large for number of rounds."
    ...
    while (roundTeams.length < numRounds) {
      ...
      while (unUsedPeople.length) {
        ...
        while (team.length < teamSize) {
          ...
          if (!remainingOptions.length) { return createTeams(teamSize, numRounds, realPeople) } // deal with random selection overlap
