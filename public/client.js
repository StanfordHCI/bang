...
$(function () {
    ...
    document.title = "Estimation Task";
...
// equivalent of initiate experiment when waitChatOn === false PK: change this?
    ...
    socket.on('initiate round', data => {
        ...
        setTimeout(() => {
            ...
            log("<strong>Reminder</strong>: You will receive the bonus pay at an hourly rate of $10/hour <strong>only</strong> if you stay for " +
                "all three rounds, answer all survey questions, and use NO external resources. This should take no more than " + totalLengthString + " total.");
            ...
            log("You will have <strong>" + textifyTime(data.duration) + "</strong> to read instructions and discuss estimations with your group.");
        }, 500);
    ...
    socket.on('timer', data => {
        log("<strong>You'll be done with this round in about " + textifyTime(data.time) + ". Please finish up discussions.</strong>");
    });
