...
function renderChats(batch) {
  fs.readFile(dir + batch + "/" + "chats.json", (err, chatsJSON) => {
    if (err) {
      ...
      try {
        const chats = JSON.parse(String(chatsJSON));
        // console.log("\nChats for batch:",batch);
        set(chats.map(a => a.round))
          .sort()
    ...
    .forEach(f => {
      fs.readFile(dir + f + "/" + "users.json", (err, usersJSON) => {
        if (err) {
          ...
          try {
            const allUsers = JSON.parse(String(usersJSON));
            allUsers.forEach(u => {
    ...
    .forEach(f => {
      fs.readFile(dir + f + "/" + "users.json", (err, usersJSON) => {
        if (err) {
          ...
          const users = JSON.parse(String(usersJSON));
          try {
            if (users[0].rooms.length == 0) {
              fs.readFile(dir + f + "/" + "chats.json", (err, chatJSON) => {
                if (err) {
                  ...
                  const chats = JSON.parse(String(chatJSON));
  ...
  fs.readFile(dir + batch + "/" + "users.json", (err, usersJSON) => {
    if (err) {
      ...
      try {
        ...
        console.log(
          "File ending error in batch",
          batch,
          JSON.parse(String(usersJSON))
        );
      }
    }
  });
    ...
    .filter(f => {
      fs.readFile(dir + f + "/" + "batch.json", (err, batchJSON) => {
        if (err) {
          ...
          const batch = JSON.parse(String(batchJSON))[0];
          if (batch) {
