...
export const joinBatch = async function(data, socket, io) {
  try {
    ...
    if (
      ...
      batch.users.length < batch.teamSize ** 2 &&
...
export const loadBatch = async function(data, socket, io) {
  try {
    ...
    if (batch.status === "waiting") {
      ...
      const finalSurveyCheck = await Survey.findOne({
        user: socket.userId,
        batch: data.batch,
        isPost: true
      })
...
const startBatch = async function(batch, socket, io) {
  try {
    ...
    for (let i = 0; i < numRounds; i++) {
        ...
        team.users.forEach(user => {
          prsHelper.push(
            User.findByIdAndUpdate(user.user, {
              $set: {
                fakeNick: user.adjective + user.animal,
                fakeAdj: user.adjective,
                fakeAnimal: user.animal,
                currentChat: team.chat
              }
            })
          );
          return user;
      ...
      const startRoundInfo = {
        rounds: rounds,
        currentRound: roundObject.number
      };
      batch = await Batch.findByIdAndUpdate(batch._id, { $set: startRoundInfo })
        .lean()
        .exec();
      ...
      await timeout(
        batch.roundMinutes *
          (1 - task.steps[task.steps.length - 1].time) *
          60000
      );
    ...
    if (process.env.MTURK_MODE !== "off" && newSurvey.isPost) {
      const [batch, user] = await Promise.all([
        Batch.findById(newSurvey.batch)
          .select("roundMinutes numRounds surveyMinutes")
          .lean()
          .exec(),
        User.findById(newSurvey.user)
          .select("systemStatus mturkId")
          .lean()
          .exec()
      ]);
      if (!batch) {
