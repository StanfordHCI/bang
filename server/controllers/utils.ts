...
const scheduleQuals = runningLive
  ...
  : [];
const mainQuals = runningLive
  ? [
      quals.onlyUSA,
      quals.hitsAccepted(100),
      quals.canJoinFrameBang,
      quals.notHasBanged,
    ]
  : [];
  ...
  roundPairs.forEach((pair) => {
    pair.forEach((roundNum, indInPair) => {
      ...
      if (generateIndividual) {
        ...
        Array.from(Array(teamSize).keys()).forEach((user) =>
          teams.push({ users: [user] })
        );
        round.teams = teams;
...
function getRandomInt(min, max, excluded) {
  try {
    ...
    return excluded.indexOf(result) === -1
      ? result
      : getRandomInt(min, max, excluded);
  } catch (e) {
    return min;
  }
}
...
export const bestRound = async (batch) => {
  ...
  for (let i = 0; i < currentRound - 1; ++i) {
    ...
    const surveys = await Survey.find({
      batch: batch._id,
      round: i + 1,
      surveyType: "midsurvey",
    });
    const surveysCount = surveys.length;
    const answerTypes = batch.tasks[i].survey
      ? batch.tasks[i].survey.map((surv) => surv.type)
      : [];
    let medianScore = 0;
    if (batch.tasks[i].survey) {
      batch.tasks[i].survey.forEach((surv) => {
        if (surv.type === "select") {
          medianScore +=
            surv.selectOptions
              .map((option) => parseInt(option.value) + 1)
              .reduce((a, b) => a + b) / surv.selectOptions.length;
        }
      });
    }
    ...
    try {
      if (surveys) {
        ...
        const questionResults = questions
          .map((x) =>
            x.map((q, index) => {
              return answerTypes[index] === "select" && parseInt(q.result) + 1
                ? parseInt(q.result) + 1
                : 0;
            })
          )
  ...
  return {
    ...
    scores: points,
    expRounds: [currentRound, result + 1],
    medianScores: medianScores,
    currentRound: currentRound,
    excludedRounds: excludedRounds,
  };
...
export const worstRound = async (batch) => {
  ...
  const result = roundFromFunction(func, {
    points: bestRoundResults.scores,
    medianScores: bestRoundResults.medianScores,
    currentRound: currentRound,
    excludedRounds: bestRoundResults.excludedRounds,
  });
...
const roundFromFunction = (func, data) => {
  // if data.points[i] === 0 or undefined, it can't win
  let result = 0;
...
export const addUnmaskedPairs = async (batch, numRound, surveyRound) => {
  ...
  const onlyMutual = (x) => {
    ...
    let result = [];
    x.forEach((y, ind) => {
      for (let i = ind + 1; i < x.length; ++i) {
        // check every element for every element, if they are (x, y) and (y, x)
        if (i === ind) continue;
        if (
          y[0].toString() === x[i][1].toString() &&
          y[1].toString() === x[i][0].toString()
        ) {
  ...
  surveys.forEach((x) => {
    ...
    x.questions.forEach((y) => {
      ...
      if (result[1]) {
        // true if result has format "int userId". True only for attractiveness questions
...
export class SpecificQuestionHandler {
  ...
  resolveQuestion(text, selectedOption) {
    ...
    if (!selectedOptionNum) {
      // if there is not such number of chosen options we select the last possible option
