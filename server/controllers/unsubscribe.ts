...
export const unsubscribe = async function (req, res) {
  try {
    ...
    if (userOfInterest === null || userOfInterest === undefined) {
      ...
      return;
    } else if (userOfInterest.batch) {
      res.json({user: `${req.params.id}`, error: 3});
      return;
    } else if (userOfInterest.systemStatus === "willbang") {
      ...
      logger.info(module, `Successful unsubscription of MTurkId= ${req.params.id}`)
    } else {
