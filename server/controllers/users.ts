...
export const activeCheck = async function(io) {
  try {
    ...
    io.to("waitroom").emit("clients-active", {
      activeCounter: counter.waitchat,
      batchReady: !!batch,
      limit: limit,
    });
    logger.info(
...
export const vote = async function(data, socket, io) {
  ...
  const oldPoll = await Survey.findOne({
    user: userId,
    round: round,
    batch: batchId,
    surveyType: "poll",
  ...
  if (data.value !== null) {
    // if data.value === null, we are just getting the information, not changing anything
    if (!oldPoll) {
      const newPoll = {
        user: userId,
        batch: batchId,
      ...
      // poll was already created, we edit the result value
      const question = oldPoll.questions.find((q) => q.type === "primary");
      await Survey.findByIdAndUpdate(oldPoll._id, {
        questions: question
          ? [{ result: data.value, type: data.type }]
          : [...oldPoll.questions, { result: data.value, type: data.type }],
      });
    }
  }

  const teammatesIds = batch.rounds[round - 1].teams
    .find((x) => {
    ...
    .users.map((z) => z.user);
  const teammatesSockets = Object.values(io.sockets.sockets).filter(
    (x: any) => {
      const userId = x.userId ? x.userId.toString() : null;
      return teammatesIds.indexOf(userId) > -1;
    }
  );
  const polls = await Survey.find({
    pollInd: pollInd,
    round: round,
    batch: batchId,
    surveyType: "poll",
    user: { $in: teammatesIds },
  });
  ...
  teammatesSockets.forEach((x: any) => {
    // send signal to every user
    return x.emit("voted", resultData);
  });
  // socket.emit('voted', resultData);
};

export const savePolls = async function(data, socket, io) {
  const batch = data.batch;
  const userId = socket.userId;
  const round = batch.currentRound;
  const batchId = batch._id;
