...
export const loadSurveyList = async function(req, res) {
  try {
    ...
    const surveyList = await SurveyTemplate.find({})
      .select(select)
      .lean()
      .exec();
...
export const loadSurvey = async function(req, res) {
  try {
    const survey = await SurveyTemplate.findById(req.params.id)
      .lean()
      .exec();
...
export const cloneSurvey = async function(req, res) {
  try {
    let original = await SurveyTemplate.findById(req.body._id)
      .lean()
      .exec();
    delete original._id;
    ...
    res.json({
      survey: { _id: survey._id, name: survey.name, teamSize: survey.teamSize },
    });
  } catch (e) {
