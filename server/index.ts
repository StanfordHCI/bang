...
export const io = require("socket.io").listen(
...
cron.schedule("*/3 * * * *", async function() {
  const batch = await Batch.findOne({ status: "waiting" })
    .select("users createdAt preChat")
    .populate("users.user")
    .lean()
    .exec();
  if (batch) {
    ...
    batch.users.forEach((item) => {
      ...
      if (
        ...
        (!user.lastCheckTime ||
          moment().diff(moment(user.lastCheckTime), "second") > 180)
      ) {
        //kick user
        prs.push(
...
if (process.env.MTURK_MODE !== "off") {
  cron.schedule("*/4 * * * *", async function() {
    try {
      const batches = await Batch.find({ status: "waiting" })
        .select(
          "tasks createdAt teamSize roundMinutes numRounds HITTitle surveyMinutes users withAutoStop"
        )
        .sort({ createdAt: 1 })
        .populate("users.user")
        .lean()
      ...
      if (currentHIT) {
        ...
        if (as && as.length)
          for (let i = 0; i < as.length; i++) {
            ...
            if (!check) {
              //add user to db and give willbang qual
              const url =
                process.env.HIT_URL +
                "?assignmentId=" +
              ...
              logger.info(
                "module",
                "User added to db, qual added, notify sent: " +
                  assignment.WorkerId
              );
            }
          }
