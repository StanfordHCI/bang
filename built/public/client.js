...
$(function () {
    ...
    var COLORS = ['#e21400', '#91580f', '#dfe106', '#ff8300', '#58dc00', '#006400', '#a8f07a', '#4ae8c4', '#ff69b4', '#3824aa', '#a700ff', '#d300e7'];
    var colorAssignment = [];
    //toggles
    ...
    socket.on('chatbot', function (data) {
        var questions = data;
        var questionIndex = permute(questions.length - 1).concat([questions.length]);
        var index = 0;
        var typingTimer;
    ...
    socket.on('initiate round', function (data) {
        ...
        $inputMessage.keydown(function (event) {
            ...
            if (event.keyCode === $.ui.keyCode.SPACE || event.keyCode === $.ui.keyCode.ENTER) {
                var terms_typed = $("#inputMessage").val().split(" ");
                ...
                if (fuzzyMatches_1.length === 1 && fuzzyMatches_1[0] !== undefined) {
                    var current_text = $("#inputMessage").val().split(" ");
                    current_text.splice(-1, 1);
                    var joined_text = current_text.join(" ");
                    if (current_text[0] === undefined) {
    ...
    socket.on('message clients', function (message) { log(message); });
    ...
    socket.on('finished', function (data) {
        ...
        HandleFinish(finishingMessage = data.message, mturk_form = mturkVariables.turkSubmitTo +
            "/mturk/externalSubmit", assignmentId = mturkVariables.assignmentId, finishingcode = data.finishingCode);
        LeavingAlert = false;
        // socket.disconnect(true);
    });
...
var getUrlVars = function (url) {
    var myJson = {};
    url.slice(url.indexOf('?') + 1).split('&').forEach(function (varString) {
