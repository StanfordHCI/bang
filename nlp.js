...
for (let i = 0; i < chatlogArr.length; i++) {
  ...
  }; //send id and timestamp with message
  if (!map.has(logIdObj)) {
...
function getToneChat(k, utterances) {
  ...
  toneAnalyzer.toneChat(toneChatParams, function(error, analysis) {
    if (error) {
      ...
      db.tonechat.insert(
        { runKey: JSON.parse(k), toneChatAnalysis: analysis },
        (err, inserted) => {
          if (err)
...
function getTone(key, chatlogText) {
  ...
  toneAnalyzer.tone(params, function(error, response) {
    if (error) {
      ...
      db.tone.insert(
        { runKey: key, toneAnalysis: response },
        (err, inserted) => {
          if (err)
            console.log("There's a problem adding a message to the DB: ", err);
          else if (inserted) console.log("Tone analysis added to the DB");
