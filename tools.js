...
const createTeams = (teamSize, numRounds, people, extraRoundOn) => {
  ...
  console.log(
    !extraRoundOn && people.length != teamSize ** 2
      ? "Can't create teams. Wrong number of people"
      : "Building teams"
  );
  ...
  while (roundTeams.length < numRounds) {
    ...
    while (unUsedPeople.length) {
      ...
      while (team.length < teamSize) {
        let teamCollaborators = team
          .map(member => collaborators[member])
          .reduce((a, b) => a.concat(b))
          .set(); //find all prior collaborators
        ...
        if (!remainingOptions.length) {
          return createTeams(teamSize, numRounds, realPeople, extraRoundOn);
      ...
      team.forEach(member => {
        collaborators[member] = collaborators[member].concat(team).set();
  ...
  if (!teamChecker(roundTeams)) {
    console.log("teams not valid");
  }
  if (extraRoundOn) {
    //couldn't get this to work generically with letters[]
